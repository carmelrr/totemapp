...
export function subscribeFeedbacksForRoute(routeId, callback) {
  const feedbacksRef = collection(db, 'routes', routeId, 'feedbacks');
  return onSnapshot(feedbacksRef, snapshot => {
    const feedbacks = snapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
...
export async function migrateFeedbacksWithDisplayName(userId = null) {
  try {
    ...
    for (const routeDoc of routesSnapshot.docs) {
      ...
      if (userId) {
        const userFeedbackQuery = query(feedbacksRef, where('userId', '==', userId));
      ...
      for (const feedbackDoc of feedbacksSnapshot.docs) {
        ...
        try {
          ...
          const feedbackRef = doc(db, 'routes', routeDoc.id, 'feedbacks', feedbackDoc.id);
          batchPromises.push(
...
export function calculateAverageStarRating(feedbacks) {
  ...
  const validRatings = feedbacks.filter(feedback => 
    feedback.starRating && feedback.starRating >= 1 && feedback.starRating <= 5
  );
  
  if (validRatings.length === 0) return 0;
  
  const totalStars = validRatings.reduce((sum, feedback) => sum + feedback.starRating, 0);
  ...
  try {
    ...
    if (operation === 'add') {
      ...
      if (feedbackData.starRating) {
        ...
        // Recalculate average
        const sum = currentStats.allStarRatings.reduce((acc, rating) => acc + rating, 0);
        currentStats.averageStarRating = sum / currentStats.allStarRatings.length;
      }
      ...
      if (feedbackData.suggestedGrade) {
        ...
        if (currentStats.highestGrade === 'N/A' || 
            (gradeValues[feedbackData.suggestedGrade] || 0) > (gradeValues[currentStats.highestGrade] || 0)) {

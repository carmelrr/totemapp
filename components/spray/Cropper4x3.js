...
import { View, StyleSheet, Dimensions, Text, TouchableOpacity, Alert, Image as RNImage } from 'react-native';
...
const Cropper4x3 = ({ imageUri, onCropComplete, style }) => {
  ...
  useEffect(() => {
    ...
    const s = Math.max(cropFrameWidth / orig.width, cropFrameHeight / orig.height);
  ...
  const handleCrop = async () => {
    try {
      ...
      console.log('[crop] S, offset:', scale.value, offsetX.value, offsetY.value);
      ...
      if (!orig.width || !orig.height || !baseImageSize.width || !baseImageSize.height) {
        console.error('[crop] Invalid dimensions - orig:', orig, 'base:', baseImageSize);
      ...
      console.log('[crop] before clamp:', { originX, originY, widthPx, heightPx });
      ...
      console.log('[crop] after clamp:', { originX, originY, widthPx, heightPx });

      if (widthPx <= 0 || heightPx <= 0) {
        console.error('[crop] Invalid crop dimensions after clamp:', widthPx, heightPx);
      ...
      if (longSide > 6000) {
        ...
        console.log('[crop] adjusted crop for resize:', { originX, originY, widthPx, heightPx });
      }
      ...
      if (onCropComplete) {
        onCropComplete(result.uri, { originX, originY, width: widthPx, height: heightPx });
      }
    } catch (error) {
  ...
  return (
    <View style={[styles.container, style]}>
      ...
      <View style={[styles.cropFrame, { width: cropFrameWidth, height: cropFrameHeight }]}>
        {/* Image with gesture handling */}
        <GestureDetector gesture={composedGesture}>
          <View style={[styles.imageContainer, { width: cropFrameWidth, height: cropFrameHeight }]}>
            ...
            {!ready && (
              <RNImage
                ...
                onError={(e) => {
                  console.error('[cropper] Hidden probe load error:', e.nativeEvent);
                }}
              />
            )}
          </View>
        ...
        <View style={styles.cropOverlay}>
          ...
          <Text style={styles.cropText}>4:3 Crop Area{'\n'}Pinch to zoom • Drag to pan</Text>

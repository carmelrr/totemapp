...
export const sprayApi = {
  ...
  async getCurrentSeason(wallId) {
    try {
      ...
      const seasonDoc = await getDoc(doc(db, 'sprayWalls', wallId, 'seasons', currentSeasonId));
      return seasonDoc.exists() ? { id: seasonDoc.id, ...seasonDoc.data() } : null;
    } catch (error) {
  ...
  listenRoutes(wallId, seasonId, callback) {
    const routesRef = collection(db, 'sprayWalls', wallId, 'seasons', seasonId, 'routes');
    ...
    try {
      ...
      if (wallDoc.exists()) {
        ...
        if (wallData.currentSeasonId) {
          const prevSeasonDoc = await getDoc(doc(db, 'sprayWalls', wallId, 'seasons', wallData.currentSeasonId));
      ...
      await setDoc(doc(db, 'sprayWalls', wallId, 'seasons', seasonId), seasonData);
  ...
  async createSend(wallId, seasonId, routeId, userId, userName) {
    try {
      const sendsRef = collection(db, 'sprayWalls', wallId, 'seasons', seasonId, 'sends');
  ...
  async getLeaderboard(wallId, seasonId = null) {
    try {
      ...
      if (seasonId) {
        // Current season only
        sendsRef = collection(db, 'sprayWalls', wallId, 'seasons', seasonId, 'sends');
      } else {

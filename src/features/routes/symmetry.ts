...
export function applySymmetryToHold(
  ...
  switch (symmetryType) {
    case 'vertical':
      newPosition = reflectPointVertical({ x: hold.x, y: hold.y }, symmetryParams.centerX);
      break;
      
    case 'horizontal':
      newPosition = reflectPointHorizontal({ x: hold.x, y: hold.y }, symmetryParams.centerY);
      break;
...
export function applySymmetryToHolds(
  ...
  return holds.map(hold => applySymmetryToHold(hold, symmetryType, symmetryParams));
}

...
export default function RouteFeedbackView({ route, onClose, isAdmin }) {
      ...
      onPanResponderMove: (evt, gestureState) => {
        ...
        const newHeight = Math.max(
          ...
          Math.min(
            ...
            startingHeight.current - gestureState.dy,
          ),
        );
  ...
  const selectMentionUser = (user) => {
    if (currentMentionStart !== -1) {
      ...
      const afterMention = comment.slice(
        currentMentionStart + 1 + currentMentionQuery.length,
      );
    ...
    while ((match = mentionRegex.exec(commentText)) !== null) {
      ...
      try {
        ...
        const user = users.find(
          (u) => u.displayName.toLowerCase() === mentionedName.toLowerCase(),
        );
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      // Extract tagged users from comment text using @ mentions
      const taggedUsersFromComment =
        await extractTaggedUsersFromComment(comment);
              ...
              <View style={styles.taggedUsersList}>
                {feedback.taggedUsers.map((taggedUser, idx) => (
                  <Text key={idx} style={styles.taggedUserName}>
                    ...
                    {idx < feedback.taggedUsers.length - 1 ? ", " : ""}
                  </Text>
                ))}
              </View>
            </View>
          ...
          <View style={styles.routeInfo}>
            <View
              style={[
                ...
                { backgroundColor: route.color || "#f0f0f0" },
              ]}
            />

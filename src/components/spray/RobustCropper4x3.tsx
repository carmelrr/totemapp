...
const RobustCropper4x3 = ({ imageUri, onCropComplete, style, enableDebug = true, onDebugLog }) => {
  ...
  const log = useCallback((...args) => {
    ...
    if (onDebugLog) {
      try { onDebugLog([prefix, ...args].map(a => (typeof a === 'object' ? JSON.stringify(a) : String(a))).join(' ')); } catch(e) { /* ignore */ }
    }
  }, [enableDebug, onDebugLog]);

  const warn = useCallback((...args) => { if (enableDebug) console.warn(`[RobustCropper][${sessionIdRef.current}]`, ...args); }, [enableDebug]);
  const err = useCallback((...args) => { if (enableDebug) console.error(`[RobustCropper][${sessionIdRef.current}]`, ...args); }, [enableDebug]);
  ...
  const progressiveDownscale = useCallback(async (srcUri, startW, startH) => {
    let w = startW;
    let h = startH;
    let uri = srcUri;
    const steps = [];
    ...
    while (Math.max(w, h) > TARGET_MAX_DIMENSION * 1.6 || (w * h) > TARGET_MAX_PIXELS * 1.6) {
      const factor = 0.6;
    ...
    if (maxSide > TARGET_MAX_DIMENSION || pixelCount > TARGET_MAX_PIXELS) {
      const scaleFactor = Math.min(TARGET_MAX_DIMENSION / maxSide, Math.sqrt(TARGET_MAX_PIXELS / pixelCount));
  ...
  const calculateInitialTransform = useCallback((dims) => {
    ...
    const safeScale = !Number.isFinite(neededScale) || neededScale <= 0 ? 1 : neededScale;
    ...
    const setupImage = async () => {
      try {
        ...
        if (imageUri.startsWith('http')) {
          ...
          log('Download complete ->', localUri, 'size(bytes)=', downloadResult?.headers?.['Content-Length']);
        }
  ...
  if (!fileInfo.exists) throw new Error('File does not exist after download');
        ...
        if (fileInfo.size > 50 * 1024 * 1024) { // 50MB
          ...
          try {
            ...
            log('Pre-resize success. New URI:', processedUri, 'newWidth:', resizeResult.width, 'newHeight:', resizeResult.height);
          } catch (resizeError) {
  ...
  log('Getting image dimensions (safe method)...');
        ...
        const getImageDimensionsSafe = async () => {
          ...
          try {
            log('Attempting ImageManipulator.manipulateAsync([]) to read dimensions');
            const result = await ImageManipulator.manipulateAsync(
              processedUri,
              [], // No operations - just get info
              { format: ImageManipulator.SaveFormat.JPEG }
            );
            log('Manipulator dimension result:', result.width, 'x', result.height, 'tempResultUri:', result.uri !== processedUri ? result.uri : 'same-as-source');
            ...
            warn('ImageManipulator dimension attempt failed:', manipulatorError.message);
            ...
            warn('Fallback dimensions selected:', defaultWidth, 'x', defaultHeight);
        ...
        try {
          ...
          log('Final chosen dimensions (original):', dimensions.width, 'x', dimensions.height);
          if (!isMounted) return;
          ...
          // Evaluate memory risk
            ...
            log('Estimated decompressed RGBA size (MB):', estimatedDecompressedMB.toFixed(2));
          ...
          if (needsDownscale) {
            ...
            try {
              const ds = await progressiveDownscale(workingUri, workingDims.width, workingDims.height);
              workingUri = ds.uri;
              workingDims = { width: ds.width, height: ds.height };
              downscaled = true;
              const newPixels = ds.width * ds.height;
              log('Downscaled result:', ds.width, 'x', ds.height, 'pixels:', newPixels, 'estMB:', ((newPixels * 4)/(1024*1024)).toFixed(2));
            } catch (dErr) {
              err('Progressive downscale failed. Continuing with original (risk of crash):', dErr);
            }
          }
          ...
          if (workingDims.width > 0 && workingDims.height > 0) {
            ...
            if (downscaled) {
              ...
              log('Post-downscale decompressed MB estimate:', newEstMB.toFixed(2));
            }
          } else {
          ...
          Alert.alert('Error', `Failed to load image dimensions: ${error.message}`);
        }
      } catch (error) {
        if (!isMounted) return;
  ...
  const panGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      try {
        ...
        if (!Number.isFinite(maxX) || !Number.isFinite(maxY) || !Number.isFinite(minX) || !Number.isFinite(minY)) {
  ...
  const rawScale = (event && Number.isFinite(event.scale) && event.scale > 0) ? event.scale : 1;
  ...
  const safeFocalX = (event && Number.isFinite(event.focalX)) ? event.focalX : frameCenterX;
  const safeFocalY = (event && Number.isFinite(event.focalY)) ? event.focalY : frameCenterY;
  ...
  log('--- Crop start ---');
      ...
      if (!imageLoaded || !localImageUri || !originalDimensions.width || !originalDimensions.height) {
        Alert.alert('Error', 'Image not ready for cropping. Please wait for the image to load completely.');
        return;
      }
      ...
      if (!Number.isFinite(currentScale) || currentScale <= 0) {
        Alert.alert('Error', 'Invalid zoom level detected. Please reset and try again.');
        return;
      }

      if (!Number.isFinite(currentTranslationX) || !Number.isFinite(currentTranslationY)) {
        Alert.alert('Error', 'Invalid position detected. Please reset and try again.');
        return;
      }
  ...
  log('Crop success. Result URI:', cropResult.uri, 'dimensions:', cropWidth, 'x', cropHeight);
  ...
  const handleReset = () => {
    ...
    log('Render: Loading state - imageLoaded:', imageLoaded, 'localImageUri:', !!localImageUri);
    return (
      <View style={[styles.container, style]}>
  ...
  log('Render: Main render - dimensions:', originalDimensions.width, 'x', originalDimensions.height, 'URI:', !!localImageUri);

  // Additional safety checks before rendering
  if (!originalDimensions.width || !originalDimensions.height || originalDimensions.width <= 0 || originalDimensions.height <= 0) {
  ...
  log('Render: About to render with minScale:', minScale, 'trackedScale:', currentScaleValue);
    ...
    <GestureHandlerRootView style={[styles.container, style]}>
          ...
          <View style={styles.imageContainer}>
            <Animated.View style={[styles.imageWrapper, imageAnimatedStyle]}>
              <Image
                ...
                onLoad={(e) => log('Image onLoad (natural size may differ platform)')} 
      ...
      <View style={styles.controls}>
        <View style={styles.buttonRow}>
          ...
          <TouchableOpacity style={[styles.button, styles.cropButton]} onPress={handleCrop}>

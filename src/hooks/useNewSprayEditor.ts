...
export const useNewSprayEditor = (options: UseNewSprayEditorOptions) => {
  ...
  const addHold = useCallback(
    (screenX: number, screenY: number) => {
      const canonicalPos = transformRef.current.screenToCanonical(
        screenX,
        screenY,
      );
  ...
  const updateHold = useCallback(
    (holdId: string, screenX: number, screenY: number, size?: number) => {
      const canonicalPos = transformRef.current.screenToCanonical(
        screenX,
        screenY,
      );
  ...
  // בחירת אחיזה קיימת
  const selectHold = useCallback(
    (screenX: number, screenY: number) => {
      const holds = routeStore.route.holds || [];
      const hitHolds = transformRef.current.getHoldsAtPoint(
        screenX,
        screenY,
        holds,
      );
  ...
  const getSelectedHold = useCallback(() => {
    ...
    return selectedIndex >= 0 && selectedIndex < holds.length
      ? holds[selectedIndex]
      : null;
